...
const FACADE_STYLES = [
  { base: '#6b5648', trim: '#57453a', masonry: 'brownstone' },
  { base: '#7d4f3e', trim: '#5e3b2e', masonry: 'brick' },
  { base: '#8d8375', trim: '#726a5c', masonry: 'limestone' },
  { base: '#61564c', trim: '#4c433b', masonry: 'brownstone' },
  // White marble (the Metropolitan Club look).
  { base: '#c9c2b4', trim: '#a8a191', masonry: 'marble' },
];
...
// Course lines and broad tonal shifts are baked into the existing colour
// map. They give a shaded facade depth without another texture lookup or a
// screen-space effect.
function paintMasonry(context, style, layout, random) {
  const { texW: width, groundBandY } = layout;
  const brick = style.masonry === 'brick';
  const marble = style.masonry === 'marble';
  // Individual brick courses are below this texture's useful resolution.
  // A faint four-pixel rhythm suggests brick without reading as clapboard.
  const courseHeight = brick ? 4 : marble ? 22 : 17;
  const blockWidth = brick ? 12 : marble ? 64 : 48;
  const lineAlpha = brick ? 0.028 : marble ? 0.055 : 0.065;

  for (let y = 21; y < groundBandY - 4; y += courseHeight) {
    // Each course is slightly different, like blocks cut from one quarry.
    context.fillStyle = `rgba(255,255,255,${random() * (brick ? 0.008 : 0.018)})`;
    context.fillRect(0, y - courseHeight + 1, width, courseHeight - 1);
    context.fillStyle = `rgba(30,25,20,${lineAlpha})`;
    context.fillRect(0, y, width, 1);

    const row = Math.round(y / courseHeight);
    const offset = row % 2 === 0 ? 0 : blockWidth / 2;
    for (let x = offset; x < width; x += blockWidth) {
      context.fillStyle = `rgba(30,25,20,${lineAlpha * (0.45 + random() * 0.25)})`;
      context.fillRect(Math.round(x), y - courseHeight + 1, 1, courseHeight - 1);
    }
  }
}

function paintDownwardShadow(context, x, y, width, depth, alpha) {
  const shadow = context.createLinearGradient(0, y, 0, y + depth);
  shadow.addColorStop(0, `rgba(24,20,17,${alpha})`);
  shadow.addColorStop(1, 'rgba(24,20,17,0)');
...
export function buildingFacade(styleIndex, seed, widthM, heightM) {
  ...
  // Low-frequency shade prevents a large wall from reading as one flat fill.
  const wallShade = context.createLinearGradient(0, 18, 0, height);
  wallShade.addColorStop(0, 'rgba(255,248,232,0.025)');
  wallShade.addColorStop(0.58, 'rgba(255,248,232,0)');
  wallShade.addColorStop(1, 'rgba(38,31,25,0.105)');
  context.fillStyle = wallShade;
  context.fillRect(0, 0, width, height);

  paintMasonry(context, style, layout, random);

  // Fine mineral variation. Soft, vertical marks read as old rain and coal
  // smoke at a distance; hard rectangular flecks looked like digital noise.
  for (let i = 0; i < 28 * layout.cols; i += 1) {
    const x = random() * width;
    const y = 18 + random() * (height - 20);
    const markWidth = 0.6 + random() * 2.2;
    const markHeight = 3 + random() * 19;
    const stain = context.createLinearGradient(0, y, 0, y + markHeight);
    stain.addColorStop(0, `rgba(35,30,25,${0.008 + random() * 0.018})`);
    stain.addColorStop(1, 'rgba(35,30,25,0)');
  ...
  for (let floor = 1; floor < layout.floors - 1; floor += 1) {
    ...
    context.fillRect(0, courseY, width, 3);
    paintDownwardShadow(context, 0, courseY + 3, width, 7, 0.12);
  }

  for (const win of layout.upper) {
    context.fillStyle = style.trim;
    context.fillRect(win.x - 4, win.y - 7, win.w + 8, 7);
    const opening = context.createLinearGradient(0, win.y, 0, win.y + win.h);
    opening.addColorStop(0, '#090807');
    opening.addColorStop(0.55, '#141310');
    opening.addColorStop(1, '#24231f');
    context.fillStyle = opening;
    context.fillRect(win.x, win.y, win.w, win.h);
    // Dark top and jambs make the painted opening behave like a recess behind
    // the instanced glass instead of another rectangle on the wall.
    paintDownwardShadow(context, win.x, win.y, win.w, 7, 0.34);
    context.fillStyle = 'rgba(5,4,3,0.24)';
    context.fillRect(win.x, win.y, 2, win.h);
    context.fillRect(win.x + win.w - 2, win.y, 2, win.h);
    context.fillStyle = style.trim;
    context.fillRect(win.x - 3, win.y + win.h, win.w + 6, 4);
    const sillShade = context.createLinearGradient(0, win.y + win.h + 4, 0, win.y + win.h + 13);
    sillShade.addColorStop(0, 'rgba(28,23,19,0.15)');
    sillShade.addColorStop(1, 'rgba(28,23,19,0)');
    context.fillStyle = sillShade;
    context.fillRect(win.x - 3, win.y + win.h + 4, win.w + 6, 9);

    if (random() < 0.42) {
      const streakX = win.x + win.w * (0.2 + random() * 0.6);
      const streakH = 7 + random() * 15;
      const streak = context.createLinearGradient(0, win.y + win.h + 4, 0, win.y + win.h + 4 + streakH);
      streak.addColorStop(0, 'rgba(36,30,24,0.075)');
      streak.addColorStop(1, 'rgba(36,30,24,0)');
      context.fillStyle = streak;
      context.fillRect(streakX, win.y + win.h + 4, 1.4, streakH);
    }
  }
  ...
  for (const win of layout.ground) {
    const opening = context.createLinearGradient(0, win.y, 0, win.y + win.h);
    opening.addColorStop(0, win.isDoor ? '#0b0908' : 'rgba(8,7,6,0.97)');
    opening.addColorStop(1, win.isDoor ? '#211d18' : 'rgba(28,25,21,0.94)');
    context.fillStyle = opening;
    context.fillRect(win.x, win.y, win.w, win.h);
  }

  // Street dirt stays concentrated at the base rather than tinting the
  // entire building. This is broad enough to survive texture minification.
  const baseGrime = context.createLinearGradient(0, height - 25, 0, height);
  baseGrime.addColorStop(0, 'rgba(35,29,23,0)');
  baseGrime.addColorStop(1, 'rgba(35,29,23,0.16)');
  context.fillStyle = baseGrime;
  ...
  paintDownwardShadow(context, 0, 20, width, 12, 0.18);
