...
function generationLabel(generation, birthplace = null) {
  if (generation === 0) return birthplace ? `immigrant, born in ${birthplace}` : 'immigrant, born abroad';
...
/**
 * Generate a complete shared game-domain patient. It has no knowledge of Blender,
...
export function generatePatient(options = {}) {
  ...
  const migrationRandom = createRandom(seed, 'migration');
  ...
  const arrivalAge = generation === 0
    ? migrationRandom.integer(3, Math.max(3, Math.min(age - 1, 35)))
    : null;
  const migration = arrivalAge == null ? null : {
    birthplace: origin.birthplace || origin.label,
    arrivalAge,
    arrivalYear: YEAR - age + arrivalAge,
  };
  ...
  return {
    ...
    identity: {
      ...
      origin: {
        id: origin.id, label: origin.label, generation,
        generationLabel: generationLabel(generation, origin.birthplace),
      },
      migration,
