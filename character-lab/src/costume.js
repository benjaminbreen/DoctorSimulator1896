...
export function createCostume(scene, bones) {
  const materials = {
    ...
    hair: new THREE.MeshStandardMaterial({ name: 'CostumeHair', color: '#0d0908', roughness: 0.92 }),
  };
  ...
  function rebuild(v) {
    ...
    const skirtGeo = ringGeometry(14, 44, (t) => {
      ...
      if (seated) {
        ...
        rx = THREE.MathUtils.lerp(0.155 * v.bodiceFit, 0.365 * v.skirtFullness, THREE.MathUtils.smoothstep(t, 0, 0.7));
        rz = THREE.MathUtils.lerp(0.125 * v.bodiceFit, 0.31 * v.skirtFullness, THREE.MathUtils.smoothstep(t, 0, 0.75));
        back = v.bustleAmount * 0.35 * (1 - t) + 0.06 * fall;
      } else {
        const eased = t * t * (3 - 2 * t);
        center.copy(P).setY(THREE.MathUtils.lerp(waistY, hemY, t));
        rx = THREE.MathUtils.lerp(0.155 * v.bodiceFit, 0.4 * v.skirtFullness, eased);
        rz = THREE.MathUtils.lerp(0.125 * v.bodiceFit, 0.34 * v.skirtFullness, eased);
        back = v.bustleAmount * 0.4 * (1 - t);
      }
    ...
    for (const side of ['L', 'R']) {
      ...
      const puff = tubeAlong(S.clone().addScaledVector(up, 0.028), E, (t) => {
        ...
        return 0.049 + 0.062 * bell * v.sleeveVolume;
      }, { lengthSegments: 14 });
    ...
    const collarTop = N.clone().lerp(H, 0.12 + 0.3 * v.collarHeight);
    ...
    const count = Math.round(v.buttonCount ?? 6);
    ...
    // head bone origin sits at the skull base; the cranium centre is ~6.5 cm above
    const skull = H.clone().addScaledVector(headUp, 0.065).addScaledVector(forward, -0.006);
    const capRadius = 0.096 * v.hairVolume;
    const cap = new THREE.SphereGeometry(capRadius, 30, 22, 0, Math.PI * 2, 0, Math.PI * 0.52);
    {
      // center part: dip vertices near the top seam
      const positionAttr = cap.attributes.position; const vertex = new THREE.Vector3();
      const partHalf = 0.008 + v.partWidth * 0.02;
      for (let index = 0; index < positionAttr.count; index++) {
        vertex.fromBufferAttribute(positionAttr, index);
        if (vertex.y > capRadius * 0.55 && Math.abs(vertex.x) < partHalf) {
          vertex.y -= 0.007 * (1 - Math.abs(vertex.x) / partHalf);
          positionAttr.setXYZ(index, vertex.x, vertex.y, vertex.z);
        }
      }
      cap.computeVertexNormals();
    }
    cap.scale(1, 0.94 * (v.hairHeight ?? 1), 1.05);
    cap.translate(skull.x, skull.y, skull.z);
    add('Costume_HairCap', cap, materials.hair, bones.head);

    const sideR = 0.044 * (v.sideVolume ?? 1);
    for (const sideSign of [-1, 1]) {
      const mass = new THREE.SphereGeometry(sideR, 18, 14);
      mass.scale(0.8, 1.0, 1.05);
      const at = H.clone().addScaledVector(right, sideSign * 0.06).addScaledVector(headUp, 0.032).addScaledVector(forward, -0.012);
      mass.translate(at.x, at.y, at.z);
      add(`Costume_HairSide_${sideSign}`, mass, materials.hair, bones.head);
    }

    const style = v.hairStyle || 'center-parted-bun';
    if (style !== 'short-parted') {
      const bunR = 0.049 * v.bunSize;
      const bun = new THREE.SphereGeometry(bunR, 20, 16);
      const at = skull.clone();
      if (style === 'low-bun') at.addScaledVector(forward, -0.075).addScaledVector(headUp, -0.055);
      else if (style === 'swept-back') { at.addScaledVector(forward, -0.045).addScaledVector(headUp, 0.075); bun.scale(1.25, 0.7, 1.1); }
      else at.addScaledVector(forward, -0.088).addScaledVector(headUp, 0.005);
