"""Build an authored patient from one Tripo/Mixamo skin plus seated clips.

Generic version of the Samuel Taylor exporter: point it at a source
directory with a skinned master FBX; a "Sitting Idle.fbx" beside it becomes
ClinicIdle and the master's own animation ships as StandingIdle. Face
shapes bake automatically; FACE_DEBUG/FACE_SHEET render review images.
"""
...
def transfer_clip(target, target_contract, path, name):
    ...
    for bone_name, parent in source_contract.items():
        if bone_name not in target_contract or target_contract[bone_name] != parent:
            raise RuntimeError(f"{os.path.basename(path)} does not match the master rig")
    ...
    print(f"TRIPO_ACTION_OK name={name} frames={frame_start}-{frame_end}")
    return action
...
def validate_action(rig, action):
    ...
    print(
        f"TRIPO_ACTION_VALID name={action.name} "
...
def main():
    ...
    rig, imported = import_fbx(os.path.join(source_dir, args.master))
    meshes = [obj for obj in imported if obj.type == "MESH"]
    if not meshes or any(
        not any(mod.type == "ARMATURE" for mod in mesh.modifiers) for mesh in meshes
    ):
        raise RuntimeError("The master must contain skinned meshes only")
    rig.name = f"{args.name}Rig"
    for index, mesh in enumerate(meshes):
        mesh.name = args.name if index == 0 else f"{args.name}_{index + 1:02d}"
    ...
    clips = []
    if os.path.exists(os.path.join(source_dir, "Sitting Idle.fbx")):
        clips.append(("Sitting Idle.fbx", "ClinicIdle"))
        clips.append((args.master, "StandingIdle"))
    else:
        clips.append((args.master, "ClinicIdle"))

    actions = [
        transfer_clip(rig, target_contract, os.path.join(source_dir, filename), name)
    ...
    print(
        f"TRIPO_EXPORT_OK name={args.name} output={output} bones={len(rig.data.bones)} "
