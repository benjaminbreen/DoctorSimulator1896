#!/usr/bin/env python3
"""Apply the inspected landmark-block contract to the generated starter spec."""
...
SPEC = Path(__file__).with_name("landmark-blocks-spec.json")


def recipe(primary: str, secondary: str, material_class: str) -> dict:
    return {
...
def action() -> dict:
    return {
        ...
        "collider": {
            ...
            "scale": [1, 1, 1],
            "isTrigger": False,
            "notes": "One coarse parcel proxy; decorative children have no colliders.",
        },
        "constraints": [],
        "destruction": {
            "breakable": False,
            "fractureGroup": "architecture",
            "seamRefs": [],
            "detachableFragments": [],
            "breakImpulse": 0,
            "debrisMaterial": "masonry",
        },
    }
...
def component(
    ...
    return {
        ...
        "level": level,
        "role": role,
        "importance": 1.0 if level == "macro" else 0.82 if level == "meso" else 0.58,
        "confidence": 0.78,
        "primitive": "box",
        "topologyClass": "assembled-solid",
        "topologyRationale": (
            "Static overlapping boxes, low-segment roofs, and instanced facade parts "
            "reproduce the visible architectural hierarchy."
        ),
        "geometryDescriptor": {
            "topologyIntent": "merged static shell with instanced repeated detail",
            "edgeTreatment": {
                "type": "small-bevel-or-layered-cornice",
                "bevelRadius": 0.02,
                "segments": 1,
            },
            "deformationStack": [],
        ...
        "colorMaterialRecipe": recipe(primary, secondary, material_class),
        "deformations": [],
        "joints": [],
        "seams": [],
        "localFeatures": [
            {"id": feature, "description": name, "evidenceRefs": ["full-object"]}
        ],
        "surfaceDetail": {
            "macroRoughness": 0.84,
            "microRoughness": 0.12,
        ...
        "details": [],
        "fidelityTier": "gameplay-landmark",
    }
...
def material(
    ...
    return {
        ...
        "albedo": {
            "dominant": color,
            "secondary": [secondary],
            "samplingNotes": "Period-reference palette interpreted through game lighting.",
        },
        "colorVariation": {
            "palette": [color, secondary],
            "pattern": "parcel-and-instance tint",
            "amplitude": 0.08,
        ...
        "textureProjection": {
            ...
            "repeat": [4, 4],
            "anisotropy": 4,
            "texelDensityIntent": "Stable masonry scale across differently sized shells.",
        },
        ...
        "roughness": {
            "base": roughness,
            "variation": 0.08,
            "map": "independent project roughness or scalar instance variation",
            "localResponse": "darker cavities are slightly rougher",
        },
        "metalness": {"base": metalness, "variation": 0.05},
        "normal": {
            "pattern": "independent project normal or geometry normals",
            "strength": 0.16,
            "scale": 24,
            "space": "tangent",
        },
        ...
        "ambientOcclusion": {
            ...
            "notes": "Applied at window, belt-course, and roof contacts.",
        },
        ...
        "localOverrides": [
            {
                "id": override,
                "region": "base-and-sill contacts",
                "roughness": min(1.0, roughness + 0.04),
                "albedoShift": "slightly darker and less saturated",
                "evidenceRefs": ["full-object"],
            }
        ],
...
def main() -> None:
    ...
    assessment["objectClass"] = {
        ...
        "formLanguage": [
            "French chateau",
            "Romanesque hotel",
            "Italian Renaissance hotel",
            "attached marble row",
            "French Renaissance mansion",
        ],
        "structureKind": [
            "static merged shells",
            "instanced facade systems",
            "low-segment roof silhouettes",
        ],
    ...
    assessment["detailInventory"] = {
        ...
        "details": [
            ...
            {"id": "netherland-crown", "kind": "contour", "description": "Tall turreted hotel crown", "mapsTo": {"type": "component.localFeatures", "ref": "mansion-rooflines"}, "evidenceRef": "full-object", "confidence": 0.82},
    ...
    spec["qualityTargets"]["fpsTarget"] = 30
    spec["qualityTargets"]["mustMatch"] = [
        "landmark silhouette and relative height",
        "street-wall continuity",
        "period material families",
        "recognizable roof and entrance systems",
    ]

    specs = [
        ("district-root", "Landmark district envelope", "macro", "static district envelope", None, "masonry", "district-envelope", "rgba(128,104,82,1)", "rgba(202,194,174,1)", "stone"),
        ("vanderbilt", "Cornelius Vanderbilt II mansion ensemble", "macro", "mansion ensemble", "district-root", "limestone", "vanderbilt-silhouette", "rgba(202,194,174,1)", "rgba(85,77,71,1)", "stone"),
        ("hotel-ensemble", "Fifth Avenue hotel ensemble", "macro", "hotel ensemble", "district-root", "brick", "hotel-height-hierarchy", "rgba(112,76,62,1)", "rgba(177,144,105,1)", "stone"),
        ("edge-ensemble", "Marble Row and edge mansion ensemble", "macro", "mansion row ensemble", "district-root", "limestone", "edge-streetwall", "rgba(214,207,190,1)", "rgba(126,92,71,1)", "stone"),
        ("vanderbilt-body", "Vanderbilt asymmetrical chateau body", "meso", "mansion mass", "vanderbilt", "limestone", "vanderbilt-massing", "rgba(202,194,174,1)", "rgba(150,142,128,1)", "stone"),
        ("new-netherland", "New Netherland tower", "meso", "hotel tower", "hotel-ensemble", "brick", "hotel-arch-bases", "rgba(104,69,58,1)", "rgba(154,105,78,1)", "stone"),
        ("savoy", "Hotel Savoy mass", "meso", "hotel mass", "hotel-ensemble", "brick", "cornice-bands", "rgba(139,93,70,1)", "rgba(194,151,106,1)", "stone"),
        ("bolkenhayn", "Bolkenhayn apartment mass", "meso", "apartment mass", "hotel-ensemble", "brick", "bolkenhayn-height", "rgba(132,84,67,1)", "rgba(188,144,103,1)", "stone"),
        ("marble-row", "Surviving Marble Row houses", "meso", "attached row houses", "edge-ensemble", "limestone", "rowhouse-party-rhythm", "rgba(222,216,201,1)", "rgba(177,169,153,1)", "stone"),
        ("huntington", "Collis P. Huntington mansion", "meso", "corner mansion", "edge-ensemble", "brick", "huntington-corner", "rgba(131,84,66,1)", "rgba(204,193,172,1)", "stone"),
        ("gerry", "Elbridge T. Gerry mansion", "meso", "corner mansion", "edge-ensemble", "limestone", "gerry-gable", "rgba(197,188,169,1)", "rgba(108,82,70,1)", "stone"),
        ("roof-systems", "Slate mansards, gables, turrets, and chimneys", "meso", "roof systems", "district-root", "slate", "mansion-rooflines", "rgba(50,55,56,1)", "rgba(67,77,72,1)", "stone"),
        ("window-bays", "Instanced sash and window bay system", "micro", "facade bay system", "district-root", "glass", "window-grid", "rgba(42,54,60,1)", "rgba(102,113,111,1)", "glass"),
        ("cornice-system", "Instanced belt course and cornice system", "micro", "cornice system", "district-root", "limestone", "cornice-bands", "rgba(216,208,190,1)", "rgba(153,143,126,1)", "stone"),
        ("dormer-system", "Instanced dormer and gable system", "micro", "dormer system", "roof-systems", "slate", "dormer-rhythm", "rgba(54,59,60,1)", "rgba(183,176,160,1)", "stone"),
        ("entrance-systems", "Arched portals, stoops, and canopies", "micro", "entrance systems", "district-root", "limestone", "entrance-portals", "rgba(208,199,180,1)", "rgba(66,55,46,1)", "stone"),
        ("ironwork-system", "Areaways, gates, and roof rails", "micro", "ironwork system", "district-root", "iron", "iron-areaways", "rgba(37,41,40,1)", "rgba(78,83,77,1)", "metal"),
    ]
    spec["componentTree"] = [component(*values) for values in specs]
    spec["materials"] = [
    ...
    spec["performanceBudget"] = {
        "qualityPriority": "performance-balanced",
        "targetTriangles": 55000,
        "maxDrawCalls": 24,
        "textureSize": 1024,
        "fpsTarget": 30,
        "optimizationPolicy": "Share shell materials, instance repeated details, disable decorative shadows, and keep one coarse collider per parcel.",
    }
    ...
    spec["assumptions"] = [
        "Hidden elevations continue the visible facade grammar.",
        "Exact lot dimensions are compressed to the existing street grid.",
        "Period photographs establish identity but do not imply measured reconstruction.",
    ]
    spec["risks"] = [
        "Tall hotel mass can dominate the compressed skyline.",
        "Excess window detail can exceed draw-call budget if not instanced.",
        "Roof shadows can hurt frame rate if enabled on every batch.",
    ]
