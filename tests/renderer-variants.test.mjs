...
test('MHR costume fitting measures the posed body and eye details follow the eye bones', async () => {
  ...
  assert.ok(eyes.groups.every((group) => /_eye$/.test(group.parent.name)), 'globes must rotate around the anatomical eye centres');
  ...
  assert.ok(eyes.groups.every((group) => group.userData.socketFit?.width > 0.015));
  ...
  assert.ok(eyes.groups[0].position.length() > 0.0001, 'identity-specific socket correction was not applied');
  assert.deepEqual(eyes.groups[0].children[0].position.toArray(), [0, 0, 0]);
